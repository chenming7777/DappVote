...
contract DappVotes {
    ...
    struct PollStruct {
        ...
        address[] voters;
        string[] avatars;
        uint[] contestantIds; // New field to store contestant IDs
    }
    ...
    function updatePoll(
        ...
        require(endsAt > startsAt,'End date must be greater than the start date');
    ...
    function contest(uint id, string memory name, string memory image) public {
        ...
        polls[id].contestantIds.push(contestant.id); // Store contestant ID
    }
    ...
    function vote(uint id, uint cid) public {
        ...
        require(currentTime() >= polls[id].startsAt && currentTime() < polls[id].endsAt, 'Voting must be in session');
